...
fn systems_consume_fuel_when_running() {
    let mut plane = Aircraft::new(
        "test",
        "Test",
        rustgear::FlightModel { mass_kg: 1000.0, wing_area_m2: 10.0, cd0: 0.04, k: 0.04, thrust_n: 1000.0 },
    );
...
fn systems_electrical_off_when_throttle_zero() {
    let mut plane = Aircraft::new(
        "test",
        "Test",
        rustgear::FlightModel { mass_kg: 1000.0, wing_area_m2: 10.0, cd0: 0.04, k: 0.04, thrust_n: 0.0 },
    );
    let mut state = FlightState::default();
